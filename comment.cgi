...
def main():
    ...
    if parameters.getvalue('content'):
        ...
        if parameters.getvalue('current_bleat_id'):
            ...
            in_reply_to = parameters.getvalue('top_bleat_id')
...
def fetch_all_bleats(): #flag_login_successfully should be change to session check
    ...
    conn = sqlite3.connect('users.db')
    #BLEAT: id, username content time longitude latitude in_reply_to
    cursor = conn.execute("SELECT username, content, time, longitude, latitude, id from bleat where username=? and in_reply_to is null order by time DESC", (self_username,))
    top_bleat_id, top_bleat_username, top_bleat_content, top_bleat_time, top_bleat_longitude, top_bleat_latitude = 0, '', '', '', '', ''
    all_comments_list = []
    ...
    for row in cursor:
        top_bleat_id = row[5]
        cursor1 = conn.execute("SELECT id from bleat where in_reply_to=?", (top_bleat_id,))
        for temp_row in cursor1:
            if temp_row[0]:
                ...
                url_avatar_top = get_avatar(top_bleat_username)
                ...
                for comment in all_comments_ordered_list:
                    ...
                    url_avatar = get_avatar(comment[2])
                    url_avatar_right = get_avatar(comment[7])
                    comment_header = ""
                    left_user = ""
                    right_user = ""
                    if comment[7] == top_bleat_username:
                        if comment[2] == self_username:
                            #comment_header = "<b> %s: </b>" % comment[2]
                            comment_header = "<b>%s Me: </b>" % url_avatar
                        else:
                            comment_header = """
                            <a href="user.cgi?username=%s">%s <b> %s: </b></a>
                            """ % (comment[2], url_avatar, comment[2])
                    else:
                        if comment[2] == self_username:
                            #left_user = "<b> %s </b>" % comment[2]
                            left_user = "<b>%s Me </b>" % url_avatar
                        else:
                            left_user = "<a href=\"user.cgi?username=%s\">%s <b> %s </b></a>" % (comment[2], url_avatar, comment[2])

                        if comment[7] == self_username:
                            #right_user = "<b> %s </b>" % comment[7]
                            right_user = "<b>%s Me </b>" % url_avatar_right
                        else:
                            right_user = "<a href=\"user.cgi?username=%s\">%s <b> %s </b></a>" % (comment[7], url_avatar_right, comment[7])
                ...
                if top_bleat_username == self_username:
                    #bleat_detail_header = "<b> %s </b>" % top_bleat_username
                    bleat_detail_header = "<b>%s Me </b>" % url_avatar_top
                else:
                    #bleat_detail_header = "<a href=\"user.cgi?username=%s\"><b> %s </b></a>" % (top_bleat_username, top_bleat_username)
                    bleat_detail_header = "<a href=\"user.cgi?username=%s\">%s <b> %s </b></a>" % (top_bleat_username, url_avatar_top, top_bleat_username)
                # fetch all of comments here

                bleats_detail += """
                <div class="panel panel-info">
                      <div class="panel-heading"><b>All comments under my bleat</b></div>
                      <div class="panel-body">
                          <div class="col-md-10 col-sm-offset-1">

                              <div class="row">
                                  <h4> %s </h4><p><small><b>time:</b>&nbsp;&nbsp;<b>posted</b> at %s &nbsp;&nbsp;&nbsp;&nbsp; <b>Location:</b> longitude: %s &nbsp;&nbsp; Latitude: %s</small></p>
                              </div>
                              <div class="row">
                                  %s
                              </div>
                              ...
                              <div class="row">
                                    <hr><h5><b>all comments below:</b></h5>
                              </div>
                              %s

                              <div class="row">
                                    <form method="post" action="comment.cgi" class="form-horizontal">
                                        <fieldset id="top%s">
                                            <!-- Textarea -->
                ...
                <hr><br>
                <script type="text/javascript">
                        var $hidden_element_store_current_username= $("<input type='hidden' name='top_bleat_id' value='%s'>");
                        $("fieldset#top%s").append($hidden_element_store_current_username);
                </script>
                """ % (bleat_detail_header, top_bleat_time, top_bleat_longitude, top_bleat_latitude, top_bleat_content, final_comment_string, single_top_id, top_bleat_id, single_top_id)
                single_top_id += 1
            break
        cursor1.close()
